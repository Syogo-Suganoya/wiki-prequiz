...
| `USE_MOCK` | サーバーのみ | **作問だけ**を切り替える。`true`（既定）で手書きの問題。記事は常に Wikipedia |
| `GEMINI_API_KEY` | サーバーのみ | Gemini 呼び出し。`USE_MOCK=false` のとき必須 |
| `GEMINI_MODEL` | サーバーのみ | 既定 `gemini-3.8-flash` |
| `WIKIMEDIA_USER_AGENT` | サーバーのみ | `WikiPreQuiz/1.0 (https://github.com/Syogo-Suganoya/wiki-prequiz)` — **常に必須** |
...
**切り替わるのは作問（AI）だけ。記事はどちらでも Wikipedia から取る。**

| `USE_MOCK` | 記事 | 問題 | 予習スキップ | 必要な設定 |
| :--- | :--- | :--- | :--- | :--- |
| `true`（既定） | Wikipedia（**`mock_data` にあるタイトルに限る**） | 手で書いた問題 | 出る | `WIKIMEDIA_USER_AGENT` |
| `false` | Wikipedia（プール全体） | Gemini が記事から作る | 出ない | ＋ `GEMINI_API_KEY` |

記事まで偽物にしない理由。手元で画面を直すときも、**本物の長さと構造**
（2,000〜20,000字、節見出しあり）で確かめたい。固定データの短い記事だけで
作ると、長文でのスクロールや読みづらさに気づけない。
Wikipedia の取得は無料で速く、課金されるのは作問だけなので、そこだけ切る。

##### モックの作問（`mock_data`）

**Wikipedia の記事を読んで、人が書いた問題を置いている。**
記事タイトルをキーにした辞書で、1記事につき3問。

固定データの問題をそのまま返していた頃は、「宇野千代を予習して太陽の問題が
出る」という噛み合わなさがあった。**予習が効くこと**はこのゲームの成立条件
そのものなので、モックでも守る。そのために、モックのときだけ
**記事を選ぶ範囲を、問題のあるタイトルに狭める**（本文はいつも通り
Wikipedia から取るので、記事そのものは実データのまま）。

途中、正規表現で穴埋めを機械生成する実装（`quizgen`）も試したが、消した。
「◯◯に入るのは？」の形にしかならず、本文の一文をそのまま出すので、
読解ではなく文字列の記憶を問うだけになる。**問題の質はゲームの質そのもの**で、
そこを機械で埋めても確かめたいことが確かめられない。数は減るが書いたものを置く。

代償は2つある。**記事の幅が10本に狭まる**こと（実データでは人気記事すべてが
対象）と、**本文が書き換わると答えが古くなりうる**こと。前者は本番の姿とは
違うので、記事まわりの見た目は `USE_MOCK=false` でも一度は見る。
後者は「現在」「最新の」といった事実を避けて書くことで減らしている。
...
必要な設定が欠けている場合は、`GET /api/health` の `missing_settings` に
変数名が並ぶ。起動は通ってしまうので、ここで気づけるようにしておく。
...
数字だけを見ると年号や順序数を拾うが、**単位を手がかりにすれば大半は落とせる**。
「人」「円」「メートル」のような量の単位が後ろに付いているものだけを数える。

* 対象: 単位のホワイトリストに載っているもの（人・円・メートル・トン・件 …）。
* 除外: 「度」「%」「KB」など、量と紛らわしい単位は**入れない**。
  「7度のノミネート」を数量として拾ってしまうため。
* 除外: 直前が「第」のもの（第2次）、直後が「目」のもの（346人目）。
* 桁: 「1億5000万キロメートル」のような漢数字混じりの連なりを解く。
* 上限: `1e15` を超える値は採用しない（天文学的数値による一発ゲーム終了の防止）。
* 下限: 抽出結果が `1,000` 未満なら `1,000`。見つからなければ 0 として同じ下限に落ちる。

**LLM を選ばなかった理由。** 抽出は `start_game` の中で要るので、
1〜3秒かかる LLM 応答を待つと設計原則 P4（リクエストの中で待たない）に反する。
実測では正規表現で **38ms** で開始できた。加えて、このモードの面白さは
「桁違いの数字で一発逆転」であって正確さではない。多少の取り違えは
ゲームを壊さないので、精度と引き換えに速さとコストゼロを取っている。

**限界も承知の上。** 実データ19件で確かめたところ、
「君のことが大大大大大好きな100人の彼女」（作品名）を数量として拾う、
俳優やドラマの記事は数量そのものが本文に無く 0 になる、といった例が出た。
前者は配点が少しずれるだけ、後者は下限の 1,000 に落ちるだけで、どちらも実害は無い。
...
> 問題数の上限3問は、固定問題が記事あたり3問だった頃の名残。
> いまはモックも実データも記事から作るので、広げる余地がある
> （60秒の予習で答えられる量か、という別の問いは残る）。
...
**最大数値は保存しない。** 本文から正規表現で導けるものなので
（[`api/app/maxnumber.py`](api/app/maxnumber.py)）、読むたびに出す。
持たせると、本文を取り直したときに数値だけ古いまま残る道ができる。
...
| 記事キャッシュ（24時間）と記事プール | ✅ |
| 最大数値の抽出（正規表現） | ✅ 実データ19件で確認 |
| Gemini による作問 | ⚠️ 実装済み。**実 API 呼び出しは未検証**（キー未設定のため） |
