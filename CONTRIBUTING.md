...
設計の意図は [DESIGN.md](DESIGN.md)、本番へ出す手順は [DEPLOY.md](DEPLOY.md)。
ここは**手元で動かして直すため**の文書。
...
* **コメントと docstring は日本語。** 何をしているかではなく、**なぜそうしたか**を書く。
  コードを読めば分かることは書かない。
* 設計原則 P1〜P6（正解を公開領域に置かない・書き込みはサーバー経由・冪等な進行・
  リクエストの中で待たない・インメモリ状態に依存しない・解答は1人に確定）は
  [DESIGN.md](DESIGN.md) にある。迷ったら原文を読む。
